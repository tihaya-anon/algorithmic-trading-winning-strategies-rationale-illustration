...
usage() {
  cat <<'USAGE'
Usage: scripts/sync-section-structure.sh [--check|--verify-rendered]
...
generate_quarto_yml() {
  ...
  {
    cat <<'YAML'
# Generated by scripts/sync-section-structure.sh.
  ...
  build:
    ...
    steps:
      ...
      - name: Verify rendered section artifacts
        run: scripts/sync-section-structure.sh --verify-rendered
