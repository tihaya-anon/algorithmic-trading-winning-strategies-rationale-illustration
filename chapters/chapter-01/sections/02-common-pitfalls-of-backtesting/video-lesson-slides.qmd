...
::: {.notes}
A safer implementation delays the raw signal so the position only uses
information available before the return is earned. The code shape is simple:
compute the raw condition, then shift it before it becomes a tradable position
or same-day order decision.
:::
...
::: {.notes}
The comparison makes the implementation risk explicit. If the backtest has a
separate code path, it can accidentally use columns, timestamps, or order
assumptions that production code never sees. If only the data feed changes,
the live constraints discipline the backtest rule.
:::
...
that later recovered and removes many names that failed.
:::
...
::: {.notes}
The backtest then acts as if the trader knew which beaten-down stocks would
survive, which is exactly the information the live trader did not have. The
table highlights why long loser portfolios can look too strong and why short
failed firms can disappear from the reported opportunity set.
:::
...
orders route to the primary exchange auction. The table separates the live
order from the historical price field the backtest must use.
:::
...
::: {.notes}
A consolidated open or close may come from a different venue, and a small
secondary-market print can be far from the auction price. A mean-reversion
backtest can then profit from a price the strategy could not actually fill.
Auction strategies need primary-exchange auction prices, or the result should
